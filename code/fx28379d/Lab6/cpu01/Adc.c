...
//--- SOC0 configuration
	AdcaRegs.ADCSOC0CTL.bit.TRIGSEL = 0;		// Trigger using software (Default 7 for ePWM2-ADCSOCA)
	...
	AdcaRegs.ADCINTSOCSEL1.bit.SOC0 = 1;		// No ADC interrupt triggers SOC0 (TRIGSEL field determines trigger)

	AdcaRegs.ADCSOC1CTL.bit.TRIGSEL = 0;        // Trigger using software
    ...
    AdcaRegs.ADCINTSOCSEL1.bit.SOC1 = 1;        // No ADC interrupt triggers SOC0 (TRIGSEL field determines trigger)
...
//--- ADCA1 interrupt configuration
	...
	AdcaRegs.ADCINTSEL1N2.bit.INT1SEL = 1;		// EOC1 triggers the interrupt
